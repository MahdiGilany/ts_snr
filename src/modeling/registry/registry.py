...
def finetune_linear_all_centers_froz_backbone(split_seed, seed=0):
    ...
    model = nn.Sequential(model.backbone.backbone.requires_grad_(requires_grad=False), model.linear_layer)
    return model
...
@register_model
def sngp_resnet10(num_deep_features=512, num_gp_features=128, mean_field_factor=25, dataset_name='UVA', **kwargs):
    ...
    sngp_model = Laplace(spectral_feature_extractor,
...
@register_model
def sngp_resnet10_vicreg_pretrained(num_deep_features=512, num_gp_features=128, mean_field_factor=25, dataset_name='UVA', **kwargs):
    if 'num_data' in kwargs:
        len_ds = kwargs['num_data'] 
        kwargs.pop('num_data')
    elif dataset_name == 'UVA':
    ...
    ckpt_name = "pretrain_spectral_resnet10_uva600_prst_stride(3,3)_no_batchnorm_last.pt" 
...
@register_model
def sngp_resnet10_vicreg_pretrained_extra_lin(num_deep_features=512, num_gp_features=128, mean_field_factor=25, dataset_name='UVA', **kwargs):
    if 'num_data' in kwargs:
        len_ds = kwargs['num_data'] 
        kwargs.pop('num_data')
    elif dataset_name == 'UVA':
    ...
    feature_extractor = nn.Sequential(OrderedDict([
        ...
        ('spectral_linear_', spectral_norm_fc(nn.Linear(512, 512), 1., 1)),
    ]))
    sngp_model = Laplace(feature_extractor,
                         num_deep_features=num_deep_features,
                         num_gp_features=num_gp_features,
                         mean_field_factor=mean_field_factor,
                         num_data= len_ds,
...
@register_model
def spectral_resnet10_vicreg_pretrained_lin_eval(num_deep_features=512, num_gp_features=128, mean_field_factor=25, dataset_name='UVA', **kwargs):
...
@register_model
def sngp_wide_resnet(num_deep_features=320, num_gp_features=128, mean_field_factor=25, dataset_name='UVA',**kwargs):
    if dataset_name == 'UVA':
        len_ds = 18095
    elif dataset_name == 'all_centers':
        len_ds = 47520
...
def spectral_resnet18(spectral_norm: bool = True, **kwargs):
    model_args = dict(**kwargs)
    model = _create_spectral_resnet18(num_classes=2, in_channels=1, spectral_norm=spectral_norm, **model_args)
    return model
