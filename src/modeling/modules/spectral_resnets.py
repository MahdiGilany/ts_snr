...
def conv_output_shape(h_w, kernel_size=1, stride=1, padding=0, dilation=1):
    ...
    h = floor( ((h_w + (2 * padding) - ( dilation * (kernel_size[0] - 1) ) - 1 )/ stride) + 1)
    w = floor( ((h_w + (2 * padding) - ( dilation * (kernel_size[1] - 1) ) - 1 )/ stride) + 1)
    return h, w
...
def spectral_fc(module, coeff: float = 1.0, n_power_iterations: int = 1, spectral_fc_active: bool = True):
...
def spectral_conv3x3(in_planes: int, out_planes: int, stride: int = 1, groups: int = 1, dilation: int = 1,
    input_size: int = 256, coeff: float = 1.0, n_power_iterations: int = 1,
    spectral_conv_active: bool = True,
) -> nn.Conv2d:
    ...
    wrapped_conv = spectral_norm_conv(
        conv3x3, coeff, input_dim, n_power_iterations
    )
    ...
    def __init__(
        ...
        if norm_layer is None:
            ...
            if spectral_conv_active:
                norm_layer = lambda num_features: spectral_bn(num_features, spectral_config.coefficient)
        if groups != 1 or base_width != 64:
            raise ValueError("SpectralBasicBlock only supports groups=1 and base_width=64")
        if dilation > 1:
            raise NotImplementedError("Dilation > 1 not supported in SpectralBasicBlock")
        
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        self.conv1 = spectral_conv3x3(inplanes, planes, stride, 
        ...
        self.conv2 = spectral_conv3x3(planes, planes, 
...
class SpectralResNet(nn.Module):
    def __init__(
        ...
        norm_layer = lambda num_features: spectral_bn(
            num_features, 
            spectral_config.coefficient, 
            spectral_bn_active=False
            )
        ...
        self.conv1 = spectral_conv(
            in_channels, self.inplanes, kernel_size=7, stride=2, padding=3,
            input_size=spectral_config.input_size,
            coeff=spectral_config.coefficient,
            n_power_iterations=spectral_config.n_power_iterations,
        ...
        self.output_size, _ = conv_output_shape(spectral_config.input_size, kernel_size=7, stride=2, padding=3)
        self.output_size, _ = conv_output_shape(self.output_size, kernel_size=3, stride=2, padding=1)
        ...
        self.fc = spectral_fc(nn.Linear(512 * block.expansion, num_classes),
                               coeff=spectral_config.coefficient,
                               n_power_iterations=spectral_config.n_power_iterations,
    ...
    def _make_layer(
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                spectral_fc(conv1x1(self.inplanes, planes * block.expansion, stride),
...
def _spectral_resnet(
    ...
    **kwargs: Any
    ...
    model = SpectralResNet(block, layers, **kwargs)
    if pretrained:
...
def spectral_resnet50(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> SpectralResNet:
    ...
    """
    return _spectral_resnet("spectral_resnet50", SpectralBasicBlock, [3, 4, 6, 3], pretrained, progress, **kwargs)


def spectral_resnet18(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> SpectralResNet:
    r"""SpectralResNet-18 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_.
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _spectral_resnet("spectral_resnet18", SpectralBasicBlock, [2, 2, 2, 2], pretrained, progress, **kwargs)
