...
class _DeepTIMeModelMAML(PLPastCovariatesModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if fast_version:
            fc = linears(layer_size, out_dim, batch_size)
        else:
            fc = nn.Linear(layer_size, out_dim)
    ...
    def forward(self, x_in: torch.Tensor) -> torch.Tensor:
        ...
        assert outdim == self.out_dim, f"The output dimension of the model {self.out_dim} must match the time-variate size {outdim}."
