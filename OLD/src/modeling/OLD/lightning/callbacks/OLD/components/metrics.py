...
from torchmetrics.functional.classification import accuracy, auroc, f1_score, recall, specificity, average_precision
...
class CollectionMetric:
    def __init__(self, pre_name, num_classes, use_all_metrics=False):
    ...
    def __call__(self, logits, labels):
        if self.use_all_metrics:
            return {
                self.pre_name + "_auc": auroc(logits, labels, num_classes=self.num_classes),
                self.pre_name + "_ap": average_precision(logits, labels, num_classes=self.num_classes),
                self.pre_name + "_acc_macro": accuracy(logits, labels, num_classes=self.num_classes, average='macro', multiclass=True),
                self.pre_name + "_sen": recall(logits, labels, num_classes=self.num_classes, average='macro', multiclass=True),
                self.pre_name + "_spe": specificity(logits, labels, num_classes=self.num_classes, average='macro', multiclass=True),
                self.pre_name + "_f1": f1_score(logits, labels, num_classes=self.num_classes, average='macro', multiclass=True),
            }
        else:
            return {
                # pre_name + '_acc_macro': Accuracy(num_classes=num_classes, average='macro', multiclass=True),
...
class GleasonMetric:
    def __init__(
        ...
        self.metric_collection = CollectionMetric(
            prefix + mode + f"_{GS_name}" + _core_prefix,
            num_classes
        )
        if GS_name == "ALL":
            self.metric_collection = CollectionMetric(
                prefix + mode + _core_prefix, num_classes, use_all_metrics=True
            )
...
class CenterMetric:
    ...
    def get_GSMetric_with_set_gs(self, set, gs) -> GleasonMetric: # former _getMetric
        if set == "val":
            gs_metric_dict = self.gs_valMetric_dict
        elif set == "test":
            gs_metric_dict = self.gs_testMetric_dict
...
class MetricManager:
    ...
    # @overload
    def update_wMetadata(self, ds_type: Literal["val", "test"], logits: torch.Tensor, labels: torch.Tensor, metadata: Dict):        
        ...
        new_dict = {k: v for k, v in metadata.items() if k in ['center', 'grade', 'psa', 'age', 'primary_grade', 'secondary_grade']}
        metadata = pd.DataFrame(new_dict)
        
        for i, center in enumerate(set(metadata["center"])):
            center_metadata = metadata.query("center == @center")
            indices = list(center_metadata.index)
            primary_grade = torch.tensor(center_metadata["primary_grade"])
            secondary_grade = torch.tensor(center_metadata["secondary_grade"])
            grade = torch.stack((primary_grade, secondary_grade))
            self.update(ds_type, center, logits[indices], labels[indices], grade)

    def compute(self, set: Literal["val", "test"], center: str = "ALL", gs: str = "ALL"):
        return self.center_metric_dict[center].compute(set, gs)

    def get_GSMetric_with_set_center_gs(self, set: Literal["val", "test"], center: str, gs: str = "ALL") -> GleasonMetric: # former _getMetric
        return self.center_metric_dict[center].get_GSMetric_with_set_gs(set, gs)
...
class CoreMetricManager(MetricManager):
    ...
    def get_logits_labels_grades_from_patchManager(
        ...
        grades_dict = {}
        for i, center in enumerate(self.cohort_specifier):
            gleason_metric = patch_manager.get_GSMetric_with_set_center_gs(set, center=center, gs="ALL")
    ...
    def update_wPatchmanager(self, patch_metric_manager: PatchMetricManager, set: Literal["val", "test"]):
        logits, labels, grades = self.get_logits_labels_grades_from_patchManager(set, patch_metric_manager)
        self.update(set, logits, labels, grades)
